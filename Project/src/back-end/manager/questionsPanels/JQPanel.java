 ...
 * 类名：JQPanel 
 * 继承：JPanel
 * 功能：判断题操作界面，用来展示题号、章节号、答案字段。功能包括对答案的增删查改。
 * 作者：王诗腾
 */
public class JQPanel extends JPanel 
{
    ...
    * 作者：王诗腾
    */
	public JQPanel(DataInputStream in,DataOutputStream out)
	{
	...
	void setQueryListener()
	{
		queryButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					out.writeUTF("ManagerJudgeQuestions-"+chapter);
					...
					modelDemo.addTableModelListener(new TableModelListener() {
						public void tableChanged(TableModelEvent e) {
							...
							String TF = (String)table.getValueAt(table.getSelectedRow(), 1);
							if(!TF.equals("T")&&!TF.equals("F")) {
								JOptionPane.showMessageDialog(null, "答案只能是T F！");
								queryButton.doClick();
								return;
							}
							//用户更新题目并确定更新
							...
							if(option==0) {
								try {
									out.writeUTF(String.format("ManagerJQUpdate-%d-%s-%s",
											chapter,(String)table.getValueAt(table.getSelectedRow(), 0),TF));
 ...
 * 功能：为JQPanel界面的数据表定制样式
 * 作者：王诗腾
 */
class JQModel extends AbstractTableModel
{
	/*表格样式类*/
	private String[][] TableData;//用来存放表格数据的线性表
	private String[] TableTitle;//表格的 列标题
	   
	public JQModel(Vector<Vector<String>> vector) 
	{
